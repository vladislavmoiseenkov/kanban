...
const updateCard = async (id, params) => {
  return (await _services_api__WEBPACK_IMPORTED_MODULE_0__["api"].patch(`/task/${id}`, params)).data;
};
...
const renderCard = card => {
  ...
  title.addEventListener('blur', async event => {
    ...
    if (newName.innerText.length && card.name !== newName.innerText) {
      await Object(_modules_Cards__WEBPACK_IMPORTED_MODULE_0__["updateCard"])(card._id, {
        name: newName.innerText
      });
...
const dragDrop = async event => {
  if (event.toElement.classList.contains('column')) {
    ...
    await Object(_modules_Cards__WEBPACK_IMPORTED_MODULE_0__["updateCard"])(cardId, {
      columnId
    });
  }
};
